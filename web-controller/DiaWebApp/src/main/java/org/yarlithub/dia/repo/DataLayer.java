...
public class DataLayer {
    ...
    public static int updateNewDevice(Device device) {
        ...
        String sql = String.format("UPDATE device "
                + "SET device_name = \"%s\", pin = \"%s\", device_mask = \"%s\", garden_id = \"%s\", schedule = \"%s\""
                + "WHERE id = \"%s\""
                , device.getDeviceName(), device.getPin(), device.getDeviceMask(),device.getGardenId(),device.getSchedule(), String.valueOf(device.getId()));
